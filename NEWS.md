...
- **Univer browser preview** — supported `docx` and `xlsx` files are loaded through the existing scoped raw-file path, converted into Univer snapshots in the browser, and rendered read-only. The parser and Univer renderer are lazy-loaded, bounded to 64 MiB source files, and expose cancel/retry/download states. Unsupported Office formats remain download-only.
...
- **Preview size confirmation** — images and PDFs require explicit confirmation at 15 MiB or larger; text, Markdown, HTML, and browser-side Univer imports use bounded loading; CSV and TSV require confirmation from 2 MiB through 15 MiB and stay download-only above 15 MiB.
...
- **Office conversion removal** — LibreOffice, `soffice`, derived Office caches, conversion routes, and Office capabilities are no longer part of the Agent/Hub runtime. Browser-side import failures never block ordinary file browsing or downloads.
