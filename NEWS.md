...
- **Workspace Search ignores** — skips common dependency / virtualenv trees by default (`renv`, `venv`, `.venv`, `node_modules`, …) and honors `.gitignore` / `.ignore`. Configure via `search_ignore` / `search_gitignore` in `agent.toml` (or `FILEBOX_AGENT_SEARCH_IGNORE` / `FILEBOX_AGENT_SEARCH_GITIGNORE`).
