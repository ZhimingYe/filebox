...
### Added
- **Remote terminal (2FA-gated)** — a new sidebar **Terminal** view opens an interactive PTY shell on the agent (explicit sign-off; crosses the read-only line). Browser I/O rides a dedicated WebSocket (`/api/agents/{id}/terminal/ws`), relayed by the hub over the agent WS (`TerminalOpen`/`Input`/`Resize`/`Close` ↔ `TerminalOpened`/`Output`/`Closed`, 16 KiB base64 chunks). The agent spawns `$SHELL` via `portable-pty` (unix-only; max 8 sessions; exactly-once close; all sessions die on disconnect). Access requires TOTP 2FA (RFC 6238, HMAC-SHA1, shared `protocol::totp` core): first use binds an authenticator via QR/manual secret (`bind/start` + `bind/confirm`), afterwards every browser session must pass `verify` — success mints an in-memory **30-min ticket bound to both the login principal and the agent**, never persisted client-side, so a browser refresh always demands a fresh code; an open pane renews the ticket every 5 min via `POST /api/terminal/2fa/renew` (no fresh code needed). Verify is per-IP rate-limited (5/30s); `terminal_2fa_bound` / `terminal_2fa_failed` / `terminal_opened` / `terminal_closed` are recorded in the login audit trail. The terminal WS validates both the session cookie and the ticket, which travels as the WebSocket subprotocol handshake header (never in the URL, so access logs and browser history can't leak it). Gated by `capabilities.terminal` (legacy agents → `unsupported_feature`). Frontend uses a lazy-loaded xterm.js pane with fit/resize, UTF-8-safe base64 I/O, and a reconnect affordance; xterm window/reporting options (DECSLPP resize, size/title reports) are disabled and OSC 52 clipboard writes are explicitly swallowed, so a remote shell can't drive browser-side features.
- **Agent-side secondary 2FA** — optional per-agent hardening: set `terminal_totp_secret` (base32) in `agent.toml` or `FILEBOX_AGENT_TERMINAL_TOTP_SECRET` and the agent advertises `capabilities.terminal_agent_2fa`, requiring every `TerminalOpen` to carry the user's current code for the agent's OWN authenticator secret (`agent_totp_code`, passed through by the hub, verified locally). Anti-replay: only a code with a strictly newer 30s counter than the last accepted one passes, and the watermark survives reconnects. Rejects with `terminal_2fa_required` / `terminal_2fa_invalid`; the UI shows a second code-entry step when the capability is set. With this enabled, a compromised hub can no longer open terminals at will — it needs a fresh, non-replayable code per open.
- **Terminal session management** — zombie recovery + runaway protection. The agent tracks per-session metadata (creation time, last input, geometry) and an idle reaper closes any session with no input for `FILEBOX_AGENT_TERMINAL_IDLE_TIMEOUT_SECS` (default 30 min, `0` disables) — a vanished browser can no longer orphan a shell forever; 5 minutes before the reap a one-shot warning is written to the session's output (re-armed by any keystroke). `GET /api/agents/{id}/terminals` lists live sessions (agent is the source of truth, so it also shows sessions the hub forgot) and `DELETE /api/agents/{id}/terminals/{req_id}` force-kills one (audited `terminal_kill`) — both need only login + CSRF, not the 2FA ticket, so recovery is never locked out. The Terminal view gains a collapsible Sessions panel (age/idle display, idle > 10 min highlighted, two-step kill) available in every 2FA phase. Output stays backpressured end-to-end (bounded 64-frame agent channel → full PTY buffer blocks the shell's write) so a flooding process can't grow memory on the agent.
...
- **Explorer workspace** — an independent, optional tree-based file view alongside the existing Files/breadcrumb workspace. It reuses previews, collections, and path copying while bounding work with virtual scrolling, two concurrent directory loads, 32 expanded folders, a 64-directory/20,000-item LRU cache, scoped refresh, and cancellation whenever the view is hidden.
...
- **Directory cache re-stat cooldown** — cache hits refresh page metadata so
  ...
  still invalidate instantly; in-place edits surface within one window of the
  last refresh.
...
- **HTML preview links work** — the preview iframe now loads the HTML file through the tokenized route in document mode: the hub injects a locked absolute `<base>`, CSP and charset meta, and an anchor fixup server-side on navigation requests, so relative links between files and `#fragment` links inside a page navigate correctly (previously they resolved to raw, frame-blocked responses or the 403 directory URL). Subresource/HEAD/XHR requests stay in the locked-down raw mode; `../` remains a hard sandbox boundary; documents over 64 MiB are refused with a clear error instead of being served.
- **HTML preview document cap raised** — document-mode HTML previews (including injected `<base>`/CSP handling) now accept files up to 64 MiB, up from 32 MiB.
...
### Fixed
- **Floating Search window can no longer be stranded off-screen** — dragging the window up previously stopped only once its *bottom* sliver (body content) was visible, which meant the header — the only drag handle — could fly entirely off the top edge with no way to pull the window back (and the lost position was persisted to localStorage). The vertical clamp now pins the header on screen, and a cancelled pointer gesture (touch / OS grab) finalizes the drag like a pointerup instead of leaving a stuck transform.
...
- **PowerPoint PDF reliability on Linux** — replace the misleading 2 GiB `RLIMIT_AS` cap with request-scoped resident process-tree accounting, raise the descriptor headroom for complex presentations, validate PDF structure before caching, discard malformed cached artifacts, and let browser decode failures force one clean reconversion instead of looping on the same bad cache.
...
### Fixed
- **Image preview / download no longer crawls at ~50 KiB then stalls** — the agent treated a single short `read()` (typical NFS rsize) as a finished WS chunk and re-opened the file for the next 50 KiB, so each round-trip re-paid metadata latency (often ~10s). Reads now loop until a full 512 KiB FileChunk or EOF, sequential chunks of the same file skip canonicalize/open/stat for a short restat window and wait for a full chunk from the in-flight fill instead of competing with it, and `O_NONBLOCK` is cleared once the leaf is a regular file. Image loading overlays now show byte progress like text/markdown.
- **PDF (and Office-to-PDF) preview no longer issues 64 KiB Range GETs** — pdf.js defaulted to `rangeChunkSize = 64 KiB`, so one document became dozens of Hub stat + Agent open round-trips on the same slow WS. The viewer now fetches the file once (session + CSRF, retry, byte progress, cancel) and hands the bytes to pdf.js, which is the same sequential 512 KiB path as images.
- **HTML preview no longer downloads the file twice** — the sandboxed iframe used to wait for a full `/api/file/raw` GET (source/warning) before requesting the same document through the preview session. Preview starts from the session URL; source view fetches on demand.
- **Raw file streams prefetch the next Agent chunk** while the current one is flushed to the browser, so preview/download no longer pauses a full Hub↔Agent round-trip between 512 KiB frames. Preview byte progress is throttled to ~100ms so the overlay stays live without a React update per TCP fragment.
