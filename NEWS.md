...
- **GET access tokens** — `POST /api/access-tokens` mints short-lived, purpose-scoped bearers for headerless GETs that cannot set CSRF headers (today: EventSource `/api/events`). The CSRF secret is never placed in URLs. File downloads and PDF.js use the normal CSRF-header path (`fetch` / pdf.js `httpHeaders`) so they do not pay an extra mint round-trip. Scope checks parse query strings with the same `form_urlencoded` decoder as axum `Query`.
